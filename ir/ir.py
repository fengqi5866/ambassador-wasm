...
class IR:
    ...
    def __init__(self, aconf: Config,
        ...
        wasm_path = os.environ.get('AMBASSADOR_WASM_DIR', "/ambassador/wasm")
        if os.path.exists(wasm_path):
            file_names = [os.path.join(wasm_path, x) for x in os.listdir(wasm_path) if
                          os.path.isfile(os.path.join(wasm_path, x)) and x.endswith(".wasm")]
            for file_name in file_names:
                name = os.path.basename(file_name)[:-5]
                self.logger.debug("IR wasm filter: " + name)
                wasm = typecast(IRFilter,
                                self.save_resource(IRFilter(self, aconf, rkey="ir.wasm",
                                                            kind="ir.wasm", name="wasm",
                                                            config={
                                                                'name': name,
                                                                'file_name': file_name
                                                            })))
                if wasm:
                    self.save_filter(wasm, already_saved=True)
        else:
            self.logger.debug("IR wasm filter: got no files in path = " + wasm_path)
